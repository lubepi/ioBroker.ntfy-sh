...
class Ntfy extends utils.Adapter {
    ...
    async onReady() {
        ...
        if (!this.config.url) {
            this.log.warn("ntfy Server URL is not configured. Please configure it in the adapter settings.");
        }
    }
    ...
    async sendNotification(msgObj) {
        ...
        if (actions) headers["Actions"] = typeof actions === "object" ? JSON.stringify(actions) : actions;
        if (markdown) headers["Markdown"] = "yes";
        if (delay) headers["Delay"] = delay;

        // Apply Authentication
        if (this.config.authType === "basic" && this.config.user && this.config.pass) {
